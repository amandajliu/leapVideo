...
window.onload = () => {
  ...
  var togglePlayGestureStart = {
    name: "playPause",
    state: "READY",
    frames: 0,
    frames_thresh: PLAY_CHANGE_THRESH,
    init_pos: null,
  };
  var toggleFullGestureStart = {
    name: "full",
    state: "READY",
    frames: 0,
    frames_thresh: FULL_CHANGE_THRESH,
    init_left: null,
    init_right: null,
  }; // [state, frames, thresh], where state = READY, STARTED, WAITING
  var volumeGestureStart = {
    name: "volume",
    state: "READY",
    init_pos: null,
    init_vol: 0,
    frames_thresh: VOL_CHANGE_THRESH,
  };
  var seekGestureStart = {
    name: "seek",
    state: "READY",
    init_pos: null,
    init_time: 0,
    frames_thresh: SEEK_CHANGE_THRESH,
  };

  var toggleFullSingleGestureStart = {
    name: "full_single",
    state: "READY",
    startFist: true,
    frames: 0,
    frames_thresh: FULL_SINGLE_THRESH,
  };

  var customGestures = [
    togglePlayGestureStart,
    toggleFullGestureStart,
    volumeGestureStart,
    seekGestureStart,
  ];
  ...
  Leap.loop(controllerOptions, function (frame) {
    ...
    for (var f = 0; f < customGestures.length; f++) {
      ...
      if (
        gestureStart.frames >= gestureStart.frames_thresh &&
        gestureStart.state == "WAITING"
      ) {
    ...
    if (
      ...
      frame.hands[0].palmPosition[2] <= CLOSE_ENOUGH_Z
    ) {
      resetState(toggleFullGestureStart, "READY");
      ...
      if (togglePlayGestureStart.state == "STARTED") {
        ...
        if (
          hand.palmVelocity[1] <= -VEL_THRESH &&
          hand.palmPosition[1] - togglePlayGestureStart.init_pos <= -Y_CHANGE
        ) {
          // toggles
          player.play();
          feedbackString = player.getState();
      ...
      if (volumeGestureStart.state == "STARTED") {
        resetState(seekGestureStart, "WAITING"); // to avoid transitioning to seeking
        var volChange =
          VOL_RATIO * (hand.palmPosition[1] - volumeGestureStart.init_pos);
        player.setVolume(
          round5(
            Math.max(0, Math.min(100, volumeGestureStart.init_vol + volChange))
          )
        );
        feedbackString =
          "setting volume: " +
      ...
      if (seekGestureStart.state == "STARTED") {
        ...
        feedbackString =
          "seeking to time: " +
          Math.floor(seekTime / 60).toString() +
          ":" +
          seconds;
      }
    } else if (
      ...
      frame.hands[0].palmPosition[2] <= CLOSE_ENOUGH_Z &&
      frame.hands[1].palmPosition[2] <= CLOSE_ENOUGH_Z
    ) {
      // only detect for one person
      hands = frame.hands;
      resetState(togglePlayGestureStart, "READY");
      resetState(volumeGestureStart, "READY");
      resetState(seekGestureStart, "READY");
      ...
      if (toggleFullGestureStart.state == "STARTED") {
        ...
        if (
          ...
          leftHand.palmPosition[0] - toggleFullGestureStart.init_left <=
            -X_CHANGE &&
          rightHand.palmPosition[0] - toggleFullGestureStart.init_right >=
            X_CHANGE
        ) {
          if (player.getWidth() == INIT_WIDTH) {
            player.resize(
              window.innerWidth - 150,
              ASPECT_RATIO * (window.innerWidth - 150)
            );
            document.getElementById("feedback").style.width =
              window.innerWidth - 150 - 10;
    ...
    if (frame.hands.length > 0) {
      for (var i = 0; i < frame.hands.length; i++) {
        ...
        handString +=
          "Direction: " + vectorToString(hand.direction, 2) + "<br />";
        handString +=
          "Palm normal: " + vectorToString(hand.palmNormal, 2) + "<br />";
        handString +=
          "Palm position: " + vectorToString(hand.palmPosition) + " mm<br />";
        handString +=
          "Palm velocity: " + vectorToString(hand.palmVelocity) + " mm/s<br />";
        handString +=
          "Sphere center: " + vectorToString(hand.sphereCenter) + " mm<br />";
        handString +=
          "Sphere radius: " + hand.sphereRadius.toFixed(1) + " mm<br />";
